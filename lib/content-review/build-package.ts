...
function exportTask(task: ChecklistTask, dueDate: string | null) {
  ...
  return {
    ...
    source: task.source ?? (task.is_custom ? "manual" : "generated"),
    relative_timing_preset: task.relative_timing_preset ?? null,
    relative_timing_label: task.relative_timing_label ?? null,
    inbox: Boolean(task.inbox),
    linked_question_ids: task.linked_question_ids ?? [],
    linked_conversation_ids: task.linked_conversation_ids ?? [],
    linked_research_ids: task.linked_research_ids ?? [],
